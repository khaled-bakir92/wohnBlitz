...
def test_chat_api():
    ...
    conv_response = requests.get(f"{base_url}/api/chat/admin/conversations", headers=headers)
    ...
    for conv in conversations:
        print(f"  - {conv['id']}: {conv['subject']} (User: {conv['user_name']}, Unread: {conv['unread_count']})")
    ...
    if conversations:
        ...
        for msg in messages:
            print(f"  - {msg['sender_type']}: {msg['message'][:50]}... ({msg['created_at']})")
