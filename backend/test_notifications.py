...
def test_notification_endpoints():
        ...
        for i, conv in enumerate(data[:3]):  # Show first 3
            print(
                f"  Conv {
                    ...
                    conv['id'][
                        :8]}..., unread={
                    conv['unread_count']}, last_message='{
                    conv.get(
                        'last_message',
                        'None')}'"
            )
    ...
    if response.status_code == 200:
        ...
        for i, conv in enumerate(data[:3]):  # Show first 3
            print(
                f"  Conv {i +
                          1}: ID={conv['id'][:8]}..., unread={conv['unread_count']}, user={conv['user_name']}"
            )
    else:
        print(f"Error: {response.text}")
    print()
    ...
    response = requests.get(
        f"{BASE_URL}/api/chat/notifications/admin/count", headers=headers
    )
    print(f"Status: {response.status_code}")
    if response.status_code == 200:
        data = response.json()
